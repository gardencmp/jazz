...
## `useAcceptInvite(onInvite)` <sub><sup>(function in `jazz-react`)</sup></sub>
...
## `createInviteLink(value, role, {baseURL?, valueHint?}?)` <sub><sup>(function in `jazz-react`)</sup></sub>
...
## `createInviteLink(value, role, {baseURL?, valueHint?}?)` <sub><sup>(function in `jazz-browser`)</sup></sub>
...
```typescript
export function loadImage(image: ImageDefinition | CoID<ImageDefinition> | {
  id: CoID<ImageDefinition>,
