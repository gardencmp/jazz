...
<CodeGroup>
    ...
    import App from "./Router.tsx"; // old
...
Let's modify `src/Router.tsx` to prepare for creating an Issue and then rendering it:
...
We have one subscriber on our Issue, with `useCoState` in `src/Router.tsx`, which will cause the `App` component and its children **to** re-render whenever the Issue changes.
...
First, we'll change `Router.tsx` to create and render `Project`s instead of `Issue`s. (We'll move the `useCoState` into the `ProjectComponent` we'll create in a second).
