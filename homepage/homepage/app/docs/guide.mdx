...
3. Modify `src/main.tsx` to set up a Jazz context:
    ...
    import { JazzReact } from "jazz-react"; // old
     // old
...
import { IssueComponent } from "./components/Issue"; // old
...
function App() { // old
    const { me } = useAccount(); // old
    const [issueID, setIssueID] = useState<ID<Issue>>( // old
        window.location.search?.replace("?issue=", "") || undefined, // old
    ); // old
    // old
    const issue = useCoState(Issue, issueID); // old
    // old
...
import { Project } from "../schema";// old
