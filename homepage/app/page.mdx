...
<Slogan>Ship apps quickly with distributed state instead of APIs & DBs.</Slogan>

<h2 className="md:mt-24">Hard things are easy now</h2>

Jazz replaces APIs, databases and message queues with a single new abstraction: **collaborative data** &mdash; giving you **distributed state** with **secure permissions built-in.**

This lets you **offer features out-of-the-box** that took best-in-class apps years to build:
...
<GridFeature icon={<UploadCloudIcon />}>On-device & cloud persistence</GridFeature>
<GridFeature icon={<PlaneIcon />}>Graceful offline/bad connection handling</GridFeature>
